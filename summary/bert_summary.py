...
# Load your dataset
df = pd.read_csv("text-summarization/datasets/test.csv")
...
df['generated_summary'] = df['article'].apply(lambda x: summarizer(x, min_length=50, max_length=500))

# Calculate ROUGE and BLEU scores for each generated summary
df['rouge_scores'] = df.apply(lambda x: compute_rouge(x['highlights'], x['generated_summary']), axis=1)
df['bleu_scores'] = df.apply(lambda x: compute_bleu(x['highlights'], x['generated_summary']), axis=1)
...
df.to_csv('summary_bert_results_2.csv', index=False)
df_avg.to_csv('summary_bert_metrics_2.csv', index=False)
